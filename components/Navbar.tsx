...
useEffect(() => {
  const fetchCollections = async () => {
    ...
    console.log("Fetched collections:", result); // should print an array
      ...
      <div className="relative flex gap-3 items-center">
        ...
        {dropdownMenu && (
          <div className="absolute top-12 right-5 flex flex-col gap-4 p-3 rounded-lg border bg-[#77c0bf] text-base-bold lg:hidden">
            <Link href="/" className="text-white hover:text-[#29465b]">Home</Link>
            <Link href={user ? "/wishlist" : "/sign-in"} className="text-white hover:text-[#29465b]">Wishlist</Link>
            <Link href={user ? "/orders" : "/sign-in"} className="text-white hover:text-[#29465b]">Orders</Link>
